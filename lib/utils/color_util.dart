...
MaterialColor getMaterialColor(Color color, {Color baseColor = Colors.white}) {
  final blend = color.alpha == 255 ? color : _convertToPaleColor(color, baseColor);
...
Color _convertToPaleColor(Color color, Color base) {
  return Color.fromARGB(((color.alpha / 255 + base.alpha / 255 * (1 - color.alpha / 255)) * 255).toInt(), _compositeColor(color.red, base.red, color.alpha), _compositeColor(color.green, base.green, color.alpha), _compositeColor(color.blue, base.blue, color.alpha));
}
